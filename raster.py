...
def read(rasters):
    ...
    for r in rasters:
        ...
        if info == None:
            ...
            if not compareRasterInfo(k, info):
                raise TypeError('all the raster must be the same projection and resolution')
    ...
    for r in rasters:
        ...
        for bidx in range(1, dataset.RasterCount + 1):
            ...
            rasterObjs.append({'bbox': bbox, 'file': r, 'band_index': bidx, 'nodata_value': nodata}) # insert key with a value of default if the key not in the dict
    ...
    for index, rast in enumerate(rasterObjs):
        arr = rast['array']
        nodata = rast['nodata_value'] = np.array([rast['nodata_value']], dtype=arr.dtype)[0]
        boolArr = (arr == nodata)
        if index == 0:
            mask = boolArr
        else:
...
def write(arr, mask, projection, resolution, bbox, nodata, path, format='GTiff'):
    ...
    dst_ds = driver.Create(path, xsize=xsize, ysize=ysize, bands=bands, eType=eType)
...
def compareRasterInfo(rast, info):
    ...
    info = {"resolution": rast.GetGeoTransform()[1], "projection": rast.GetProjection()}
    rast = None
    return info

def test():
    print(read(['/mnt/AllThings/JSY/RSEI/www/lst.tif', '/mnt/AllThings/JSY/RSEI/www/wet.tif', '/mnt/AllThings/JSY/RSEI/www/ndvi.tif', '/mnt/AllThings/JSY/RSEI/www/ndbsi.tif']))
